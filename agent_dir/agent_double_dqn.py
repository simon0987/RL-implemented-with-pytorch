...
outputFile = open('double_dqn.csv', 'w+')
...
class AgentDQN(Agent):
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            _,actions = self.online_net(non_final_next_states).max(1,keepdim=True)
            next_state_values[non_final_mask] = self.target_net(non_final_next_states).gather(1,actions).view(-1).detach()
        # TODO:
