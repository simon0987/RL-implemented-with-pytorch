...
def parse():
    ...
    parser.add_argument('--train_double_dqn',action='store_true', help='whether train double DQN')
    parser.add_argument('--train_dueling_dqn', action='store_true', help='whether train dueling DQN')
    parser.add_argument('--train_mario', action='store_true', help='whether train mario')
    parser.add_argument('--test_pg', action='store_true', help='whether test policy gradient')
    parser.add_argument('--test_dqn', action='store_true', help='whether test DQN')
    parser.add_argument('--test_double_dqn', action='store_true', help='whether test double DQN')
...
def run(args):
    if args.train_dueling_dqn:
        env_name = args.env_name or 'AssaultNoFrameskip-v0'
        env = Environment(env_name, args, atari_wrapper=True)
        from agent_dir.agent_dueling_dqn import AgentDQN
        agent = AgentDQN(env, args)
        agent.train()

    if args.train_double_dqn:
        env_name = args.env_name or 'AssaultNoFrameskip-v0'
        env = Environment(env_name, args, atari_wrapper=True)
        from agent_dir.agent_double_dqn import AgentDQN
        agent = AgentDQN(env, args)
        agent.train()
